...
var t2=new TouchSlider({id:'sliderlist', speed:600, timeout:6000, before:function(index){
    ...
    function add_cart(goodId,quantity)//商品ID，购买数量
    {
		...
		$(".group1").colorbox({rel:'group1',width:"100%",height:"100%"});
		...
		</script><div class="ware_info"><div class="ware_pic"></div><div id="slider" class="slider" style="overflow: hidden; visibility: visible; list-style: none outside none; position: relative;"><ul id="sliderlist" class="sliderlist" style="position: relative; overflow: hidden; transition: left 600ms ease 0s; width: 784px; left: 202px;"><?php if(is_array($img_list)): $i = 0; $__LIST__ = $img_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$img): $mod = ($i % 2 );++$i;?><li style="float: left; display: block; width: 596px;"><a class="group1" href="<?php echo attach(get_thumb($img['url'], '_b'), 'item');?>"><img  width="90%" height="85%" src="<?php echo attach(get_thumb($img['url'], '_b'), 'item');?>"></li><?php endforeach; endif; else: echo "" ;endif; ?></ul><div id="pagenavi"><?php if(is_array($img_list)): $k = 0; $__LIST__ = $img_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$img): $mod = ($k % 2 );++$k;?><a href="javascript:void(0);" <?php if($k == 1): ?>class="active"<?php endif; ?>><?php echo ($k); ?></a><?php endforeach; endif; else: echo "" ;endif; ?></div></div><script type="text/javascript">            var active=0,
                as=document.getElementById('pagenavi').getElementsByTagName('a');
            ...
            </script><input type="hidden" value="<?php echo ($item["id"]); ?>" id="goodId" ><div class="ware_text"><div class="rate"><h2 class="ware_title" style="margin-bottom:10px"><?php echo ($item["title"]); ?></h2><!-- Baidu Button BEGIN --><div id="bdshare" class="bdshare_t bds_tools_32 get-codes-bdshare" style="font-size:1em; height:37px; line-height:37px; float:none;margin-bottom:10px"><span style="float:left; margin-top:2px">分享到：</span><a class="bds_tsina"></a><a class="bds_qzone"></a><a class="bds_tqq"></a><a class="bds_copy"></a></div><script type="text/javascript" id="bdshare_js" data="type=tools" ></script><script type="text/javascript" id="bdshell_js"></script><script type="text/javascript">document.getElementById("bdshell_js").src = "http://bdimg.share.baidu.com/static/js/shell_v2.js?cdnversion=" + Math.ceil(new Date()/3600000)
</script><!-- Baidu Button END --><span class="letterprice">现价: </span><span ectype="goods_price" class="fontColor3">¥<?php echo ($item["price"]); ?></span><br><span class="letter1">品牌: </span><?php echo ($item["brand"]); ?><br>                销售情况: 售出 <?php echo ($item["buy_num"]); ?> 件<br>                所在地区: 中国                        
